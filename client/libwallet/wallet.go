...
import (
	...
	ed255192 "github.com/koinotice/vite/crypto/ed25519"
...
func RecoverEntropyStoreFromMnemonic(mnemonic, newPassphrase, language, extensionWord *C.char) *C.char {
	...
	em, err := tmp.RecoverEntropyStoreFromMnemonic(mnemonicStr, newPassphraseStr)
	if err != nil {
		return CString(failResult(err))
	}
...
func TransformMnemonic(mnemonic, language, extensionWord *C.char) *C.char {
	...
	//	extensionWordP = nil
	//}
	primaryAddress, err := entropystore.MnemonicToPrimaryAddr(GoString(mnemonic))
	if err != nil {
		return CString(failResult(err))
	}
	return CString(successResultWithData(primaryAddress.Hex()))
...
func ComputeHashForAccountBlock(block *C.char) *C.char {
	...
	hashBlock := ledger.AccountBlock{}
	err := json.Unmarshal([]byte(blockStr), &hashBlock)
	if err != nil {
		return CString(failResult(err))
	}
	if verbose {
		bytes, _ := json.Marshal(hashBlock)
		fmt.Println(string(bytes))
	}
	hash := hashBlock.ComputeHash()
