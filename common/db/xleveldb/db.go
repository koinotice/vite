...
import (
	...
	"github.com/koinotice/vite/common/db/xleveldb/iterator"
	"github.com/koinotice/vite/common/db/xleveldb/journal"
	"github.com/koinotice/vite/common/db/xleveldb/memdb"
	"github.com/koinotice/vite/common/db/xleveldb/opt"
	"github.com/koinotice/vite/common/db/xleveldb/storage"
	"github.com/koinotice/vite/common/db/xleveldb/table"
...
func (db *DB) Get2(key []byte, ro *opt.ReadOptions, auxm *memdb.DB, seq uint64) (value []byte, err error) {
	err = db.ok()
	if err != nil {
		return
	}

	return db.get(auxm, nil, key, seq, ro)
}
...
func (db *DB) NewIterator2(slice *util.Range, ro *opt.ReadOptions, auxm *memdb.DB, seq uint64) iterator.Iterator {
	if err := db.ok(); err != nil {
		return iterator.NewEmptyIterator(err)
	}
	...
	// Iterator holds 'version' lock, 'version' is immutable so snapshot
	// can be released after iterator created.
	return db.newIterator2(auxm, nil, seq, slice, ro)
}
