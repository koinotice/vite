...
func NewServer() *Server {
	...
	// register chain default service which will provide meta information about the RPC service such as the services and
	// methods it offers.
...
func (s *Server) RegisterName(name string, rcvr interface{}) error {
	...
	// already chain previous service register under given name, merge methods/subscriptions
...
//
// If singleShot is true it will process chain single request, otherwise it will handle
// requests until the codec returns an error when reading chain request (in most cases
...
func (s *Server) serveRequest(ctx context.Context, codec ServerCodec, singleShot bool, options CodecOption) error {
	...
	// if the codec supports notification include chain notifier that callbacks can use
	...
	for atomic.LoadInt32(&s.run) == 1 {
		...
		if err != nil {
			// If chain parsing error occurred, send an error
			if err.Error() != "EOF" {
		...
		// If chain single shot request is executing, run and return immediately
		if singleShot {
			if batch {
		...
		// For multi-shot connections, start chain goroutine to serve and loop back
		pend.Add(1)
...
// ServeSingleRequest reads and processes chain single RPC request from the given codec. It will not
// close the codec unless chain non-recoverable error has occurred. Note, this method will return after
// chain single request has been processed!
...
func (s *Server) exec(ctx context.Context, codec ServerCodec, req *serverRequest) {
	...
	// when request was chain subscribe request this allows these subscriptions to be actived
	if callback != nil {
		callback()
	}
}
...
// of requests, an indication if the request was chain batch, the invalid request identifier and an
